...
if(nHeight_FORK_BLOCK2 > FORK_BLOCK2){unsigned int nTargetSpacing = 5 * 60;}
else{ unsigned int nTargetSpacing = 1 * 60;}
...
const string strMessageMagic = "EuropeCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    } else if(nHeight >= 2 && nHeight <= 770) {
    nSubsidy = 100 * COIN;
    } else if(nHeight >= 771 && nHeight <= 1440) {
    nSubsidy = 1000 * COIN;
    } else if(nHeight >= 1441 && nHeight <= 10080) {
    nSubsidy = 10000 * COIN;
...
{
    if (pindexBest == NULL || nBestHeight < Checkpoints::GetTotalBlocksEstimate() || fImporting)
        return true;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        {
            // BitSenddev 10-04-2016 Hot Fix for Block 850242 
...
// Add Debug 18-04-2016 Bitsendev
	printf("coin age nValueIn=%"PRId64" nTimeDiff=%d bnCentSecond=%s nHeight:%d \n", nValueIn, nTime - txPrev.nTime, bnCentSecond.ToString().c_str(),pindexBest->nHeight);
            if (fDebug && GetBoolArg("-printcoinage"))
                printf("coin age nValueIn=%"PRId64" nTimeDiff=%d bnCentSecond=%s\n", nValueIn, nTime - txPrev.nTime, bnCentSecond.ToString().c_str());
        }
    }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!fImporting && pblock->IsProofOfStake() && setStakeSeen.count(pblock->GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash) && !Checkpoints::WantedByPendingSyncCheckpoint(hash))
    ...
    {
        printf("ProcessBlock: ORPHAN BLOCK Fehler Main 2328, prev=%s\n", pblock->hashPrevBlock.ToString().substr(0,20).c_str());
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "EuropeCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        if (!fImporting && !IsInitialBlockDownload())
        {
