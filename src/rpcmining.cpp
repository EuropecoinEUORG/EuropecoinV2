...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    if(pindexBest->nHeight <= FORK_BLOCK) {
        ...
        obj.push_back(Pair("stakeinterest(%)",  "0.5% - 5% variable")); // VPoS
    }
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "EuropeCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "EuropeCoin is downloading blocks...");
